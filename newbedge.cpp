...
NewbEdge::NewbEdge(NewbNode *sourceNode, NewbNode *destNode)
    :arrowSize(10)
{
...
NewbNode * NewbEdge::sourceNode() const
{
    return source;
}

NewbNode * NewbEdge::destNode() const
{
    return dest;
}
void NewbEdge::adjust()
{
    if(!source || !dest)
        return;
...
QRectF NewbEdge::boundingRect() const
{
    if(!source || !dest)
        return QRectF();
...
void NewbEdge::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    if(!source || !dest)
        return;
