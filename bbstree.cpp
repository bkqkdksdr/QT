...
{
    if(nullptr==T)return;
    if(nullptr!=T->lchild)DestroyBBST(T->lchild);
    if(nullptr!=T->rchild)DestroyBBST(T->rchild);
    free(T);
    T=nullptr;
}
...
void MergeBBSTree(BBSTree &T1,BBSTree T2)
{
    ...
    if (nullptr != T2->lchild) MergeBBSTree(T1,T2->lchild);
    InsertAVL(T1,T2->data,s);
    if (nullptr != T2->rchild) MergeBBSTree(T1,T2->rchild);
    return;
}
